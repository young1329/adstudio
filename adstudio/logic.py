...
class Logic(Device): 
    ...
    def DI_read_record(self):
        ...
        while (cSample < self.cSamples):
            dwf.FDwfDigitalInStatus(self.hdwf[self.idx], c_int(1), byref(sts))
            if (cSample ==0 and (sts==DwfStateConfig or sts==DwfStatePrefill or sts==DwfStateArmed)):
                continue  # not started
            dwf.FDwfDigitalInStatusRecord(self.hdwf[self.idx],byref(cAvailable), byref(cLost), byref(cCorrupted))
            cSample += cLost.value
            if cLost.value:
                fLost = 1
            if cCorrupted.value:
    ...
    def DI_read_buffer(self):
        ...
        try:
            ...
            Eflag=dwf.FDwfDigitalInStatusData(self.hdwf[self.idx], \
                self.rgwSamples, self.nBytes*self.cSamples )
            if ( not Eflag ):
